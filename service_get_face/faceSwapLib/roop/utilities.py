...
def normalize_output_path_for_multiple(source_path: list[list[str]], target_path: str, output_path: str) -> Optional[str]:
    if source_path and target_path and output_path:
        source_name, _ = os.path.splitext(os.path.basename(source_path[0][0]))
        target_name, target_extension = os.path.splitext(os.path.basename(target_path))
        if os.path.isdir(output_path):
...
def extract_face_using_bbox(face, frame, margin=(150,150,150,150)):
    bbox = [int(i) for i in face.bbox]
    bbox[0] = bbox[0]-margin[1] if bbox[0]-margin[1]>0 else bbox[0]
    bbox[1] = bbox[1]-margin[0] if bbox[1]-margin[0]>0 else bbox[1]
    bbox[2] = bbox[2]+margin[3] if bbox[2]+margin[3]<frame.shape[0] else bbox[2]
    bbox[3] = bbox[3]+margin[2] if bbox[3]+margin[2]<frame.shape[1] else bbox[3]
